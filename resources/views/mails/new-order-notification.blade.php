...
<head>
    ...
    <style>
        ...
        .order-list { background: #fff; border-radius: 8px; padding: 12px 12px 8px 12px; margin: 10px 0 0 0; }
...
<body>
    <div class="mail-container">
        ...
        <div class="content">
            <div class="desc">
                You have received a new order (ID: <b>{{ $order->id }}</b>) at <b>{{ $order->created_at->timezone($order->user->timezone ?? config('app.timezone'))->format('Y-m-d H:i') }}</b>.
            </div>
            <div class="card">
                <div><strong>Customer:</strong> {{ $order->user->first_name }} {{ $order->user->last_name }}</div>
                ...
                @if($order->dietary_info)
                    <div class="desc"><strong>Dietary Info:</strong></div>
